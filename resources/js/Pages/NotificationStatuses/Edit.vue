...
const confirmingNotificationStatusDeletion = ref(false);
// Get the existing data from the page props
const { props } = usePage();
const notificationStatus = props.notification_status;
...
const form = useForm({
    ...
    is_active: Boolean(notificationStatus.is_active),
});

// Function for submitting the form to the controller for updating the record
const update = () => {
    form.put(`/notification-statuses/${notificationStatus.id}`);
};

const confirmNotificationStatusDeletion = () => {
    confirmingNotificationStatusDeletion.value = true;
};

const deleteStatus = () => {
    form.delete(`/notification-statuses/${notificationStatus.id}`, {
        preserveScroll: true,
...
const closeModal = () => {
    confirmingNotificationStatusDeletion.value = false;
    form.reset();
};
</script>
            ...
            <div class="max-w-3xl rounded-md">
                <form @submit.prevent="update">
                    ...
                    <div class="flex items-center gap-4 justify-between py-10">
                        <DangerButton @click.prevent="confirmNotificationStatusDeletion">
                    ...
                    <div>
                        <Modal
                            :show="confirmingNotificationStatusDeletion"
                            @close="closeModal"
                        >
